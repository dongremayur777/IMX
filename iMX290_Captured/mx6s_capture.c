...
static int mx6s_configure_csi(struct mx6s_csi_dev *csi_dev)
{
	...
	case V4L2_PIX_FMT_SRGGB8:
	            case V4L2_PIX_FMT_SRGGB10:   // raw10
		width = pix->width;
		break;
	...
	csi_set_imagpara(csi_dev, width, pix->height);
 		...
 		printk("liutest fsl,two-8bit-sensor-mode from pixelformat V4L2_PIX_FMT_SRGGB10\n");
	if (csi_dev->csi_mipi_mode == true) {
	printk("liutest V4L2_PIX_FMT_SRGGB10 mipi mode %d\n", pix->width);
 			width = pix->width;
		cr1 = csi_read(csi_dev, CSI_CSICR1);
		cr1 &= ~BIT_GCLK_MODE;
