...
static const struct csis_pix_format mipi_csis_formats[] = {
	{
		...
		.data_alignment = 16,  //16 - Original
	}, {
		.code = MEDIA_BUS_FMT_SBGGR8_1X8,
		.fmt_reg = MIPI_CSIS_ISPCFG_FMT_RAW8,
		.data_alignment = 8,  //16 - Original
	} ,{
...
static void __mipi_csis_set_format(struct csi_state *state)
{
	...
	//pr_info("fmt: %#x, %d x %d; CONFIG:0x%x \n", mf->code, mf->width, mf->height, val); //for sync issue
...
static void mipi_csis_set_params(struct csi_state *state)
{
	...
	if (state->csis_fmt->data_alignment == 32)
		...
		val &= ~MIPI_CSIS_ISPCFG_ALIGN_32BIT;
		val |= MIPI_CSIS_ISPCFG_FMT_RAW10;
	//	val |= 1 << 12; // for sync issue - Modified
	//	pr_info(" mipi_csis_set_params before CONFIG:0x%x\n", val); // for sync issue
	mipi_csis_write(state, MIPI_CSIS_ISPCONFIG_CH0, val);
//	pr_info(" mipi_csis_set_params after CONFIG:0x%x\n", val);  // for sync issue
